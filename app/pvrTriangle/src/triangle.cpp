...
bool createEGLDisplay(EGLDisplay& eglDisplay)
{
    ...
    eglDisplay = eglGetPlatformDisplayEXT(EGL_PLATFORM_SURFACELESS_MESA, EGL_DEFAULT_DISPLAY, NULL);
...
void convertRGBAtoRGB565(const uint8_t *rgba, uint16_t *rgb565, int width, int height)
{
...
bool renderScene(GLuint shaderProgram, EGLDisplay eglDisplay, EGLSurface eglSurface)
{
	...
	// glReadPixels(0, 0, 240, 240, GL_RGBA, GL_UNSIGNED_INT_2_10_10_10_REV, pointer);
    glReadPixels(0, 0, 240, 240, colorFormat, colorType, pointer);
    if (!testGLError("glReadPixels")) { return false; }
	...
	// FILE *output = fopen("frame", "wb");
    // if (output) {
    //     fwrite(LCD_getFrameBufferAddress(), 1, 240*240*4, output);
    //     fclose(output);
    //     printf("Wrote %zu bytes to frame.raw\n", 240*240*2);
    // } else {
    //     fprintf(stderr, "Failed to open file frame.raw for writing!\n");
	// 	printf("Error %d \n", errno);
    // }
	LCD_displayFrame();
...
{
	// Renders a triangle for 8100 frames using the state setup in the previous function
	for (int i = 0; i < 100; ++i)
	{
        struct timespec render_current;
	    struct timespec render_end;
        clock_gettime(CLOCK_MONOTONIC, &render_current);

		if (!renderScene(shaderProgram, eglDisplay, eglSurface)) { return false; }
        clock_gettime(CLOCK_MONOTONIC, &render_end);
        long render_usec = (render_end.tv_sec - render_current.tv_sec) * 1000000L
        + (render_end.tv_nsec - render_current.tv_nsec) / 1000L;

        printf("render 1 frame time: %.2fms\n", render_usec / 1000.0);
