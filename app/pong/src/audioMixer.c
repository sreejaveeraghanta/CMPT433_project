...
void AudioMixer_queueSound(wavedata_t *pSound)
{
	...
	bool free_sound_slot = false;
	for (int i = 0; i < MAX_SOUND_BITES; i++){
		if(soundBites[i].pSound == NULL){
			//Empty slot, put sound in
			soundBites[i].pSound = pSound;
			soundBites[i].location = 0;
			free_sound_slot = true;
			break;
		}
	}
	...
	if(!free_sound_slot){
		printf("Error: No free sound slot found!\n");
	}
...
{
	volume -= DECREMENT;
	if(volume < AUDIOMIXER_MIN_VOLUME){
		volume = AUDIOMIXER_MIN_VOLUME;
	}
	AudioMixer_setVolume(volume);
}
...
static void fillPlaybackBuffer(short *buff, int size)
{
	...
	// 1. Wipe the buff to all 0's
	memset(buff, 0, size * sizeof(short));

	// 2. mutex for soundBites[]
	pthread_mutex_lock(&audioMutex);

	// 3. Loop through each slot in soundBites[]
	for(int i = 0; i<MAX_SOUND_BITES; i++){
		// Waiting or partial sounds
		if(soundBites[i].pSound != NULL){
			// Get local variable
			wavedata_t* local_psound = soundBites[i].pSound;
			int local_location = soundBites[i].location;

			// Add current sound across buff, stopping when all added
			for (int buff_index = 0; buff_index<size; buff_index++){
				// stopping when sound finished
				if(local_location >= local_psound->numSamples){
					break;
				}

				int sum_pcm_sample = buff[buff_index] + local_psound->pData[local_location];

				// Clip if needed
				if(sum_pcm_sample > SHRT_MAX){
					sum_pcm_sample = SHRT_MAX;
				}
				if(sum_pcm_sample < SHRT_MIN){
					sum_pcm_sample = SHRT_MIN;
				}

				buff[buff_index] = sum_pcm_sample;

				local_location++;
			}

			// Update original variable with local value
			soundBites[i].location = local_location;

			// Clear slot if finished sound, loop back if background music
			if(local_location >= local_psound->numSamples){
				if(local_psound != background_music_file_address()){
					soundBites[i].pSound = NULL;
				}
				soundBites[i].location = 0;
			}
...
{
	// Used to suppress warning https://stackoverflow.com/questions/3599160/how-can-i-suppress-unused-parameter-warnings-in-c
	(void)_arg;
